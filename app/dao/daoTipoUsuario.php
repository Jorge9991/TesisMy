...
class daoTipoUsuario implements Itipousuario {
    ...
    public function delete(\TipoUsuario $tipousuario) {
        try {
            $sql = 'delete from tb_tipousuario WHERE idTipoUsuario = :id';
            $cn = DataBase::getInstancia();
            $stmp = $cn->prepare($sql);
    ...
    public function listar() {
        $tiposusuarios = array();
        try {
            $sql = 'SELECT * from tb_tipousuario group by idtipousuario  order by idtipousuario asc';
            $cn = DataBase::getInstancia();
            $stmp = $cn->prepare($sql);
            $stmp->execute();
    ...
    public function listargestor() {
          $tiposusuarios = array();
        try {
            $sql = 'SELECT * from tb_tipousuario where idtipousuario != 4 group by idtipousuario  order by idtipousuario asc';
            $cn = DataBase::getInstancia();
            $stmp = $cn->prepare($sql);
            $stmp->execute();
            foreach ($stmp->fetchAll(PDO::FETCH_OBJ) as $obj) {
